...
this.Shopify.theme.PredictiveSearchComponent = (function (PredictiveSearch) {
  ...
  PredictiveSearchComponent.prototype._addAccessibilityAnnouncer = function () {
    ...
    this._accessibilityAnnouncerDiv.setAttribute(
      'style',
      'position: absolute !important; overflow: hidden; margin: -1px; margin-top:8px; padding: 0; border: 0;'
    );
...
theme.SearchResultsTemplate = (function () {
  ...
  function renderSalePrice(product) {
    return [
      '<dt>',
      '<span class="">' + theme.strings.salePrice + '</span>',
      '</dt>',
      '<dd>',
  ...
  function renderPrice(product) {
    return [
      '<dt>',
      '<span class="">' + theme.strings.regularPrice + '</span>',
      '</dt>',
      '<dd>',
  ...
  function renderVendor(product) {
    ...
    return [
      '<dt>',
      '<span class="">' + theme.strings.vendor + '</span>',
      '</dt>',
...
theme.Cart = (function () {
  ...
  function Cart(container) {
    ...
    this.$thumbnails.css('cursor', 'pointer').css('visibility', 'visible');
    this.$container.on(
      'click',
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
      ...
      $.post(params)
        .done(
          function (state) {
            ...
            this._setCartQuantity(state.item_count);
    ...
    _setCartQuantity: function (quantity) {
      this.$cartPopupCartQuantity =
        this.$cartPopupCartQuantity || $('[data-cart-popup-cart-quantity]');
      var ariaLabel;
...
theme.Product = (function () {
  ...
  var mediumUpQuery = '(min-width: ' + theme.breakpoints.medium + 'px)';
  var firstItem = `<tr class="cart__row" data-cart-item="" data-cart-item-key="" data-cart-item-url="" data-cart-item-title="" data-cart-item-index="1" data-cart-item-quantity="1">
        <td class="cart__meta small--text-left" data-cart-table-cell="">
            <div class="cart__product-information">
                <div class="cart__image-wrapper">
                    <img class="cart__image" src="" alt="" data-cart-item-image="" style="visibility: visible; cursor: pointer;">
                </div>
                <div>
                    <div class="list-view-item__title">
                        <a href="" class="cart__product-title" data-cart-item-title=""></a>
                    </div><ul class="product-details hide" data-cart-item-details="" aria-label="Product details"></ul>
                    <p class="cart__remove">
                        <a href="/cart/change?line=1&quantity=0" class="text-link text-link--accent" aria-label="" data-cart-remove>Entfernen</a>
                    </p>
                </div>
            </div>
        </td>
          ...
          <div data-cart-item-price>
            <dl data-cart-item-price-list>
              <div data-cart-item-regular-price-group="">
                <dt>
                    <span class="">Normaler Preis</span>
                </dt>
                <dd data-cart-item-regular-price=""></dd>
              </div>
            </dl>
          </div>
          <ul class="order-discount order-discount--list order-discount--title order-discount--cart hide" aria-label="Discount" data-cart-item-discount-list=""></ul>

          <div class="cart__qty medium-up--hide">
              <label for="updates_44061465739547:692fa26d8f8144a0fa6f6dec16f43e0d" class="cart__qty-label" aria-label="" data-quantity-label-mobile="">
              Menge
              </label>
              <input id="updates_44061465739547:692fa26d8f8144a0fa6f6dec16f43e0d" class="cart__qty-input" type="number" value="1" min="0" pattern="[0-9]*" data-quantity-input data-quantity-item="1" data-quantity-input-mobile="">
          </div>
        ...
        <td class="cart__quantity-td text-right small--hide">
            <div class="cart__qty">
                <label for="updates_large_44061465739547:692fa26d8f8144a0fa6f6dec16f43e0d" class="cart__qty-label" data-quantity-label-desktop="">Quantity</label>
                <input id="updates_large_44061465739547:692fa26d8f8144a0fa6f6dec16f43e0d" class="cart__qty-input" type="number" name="updates[]" value="1" min="0" pattern="[0-9]*" data-quantity-input="" data-quantity-item="1" data-quantity-input-desktop for="updates_large_44061465739547:692fa26d8f8144a0fa6f6dec16f43e0d">
            </div>
        ...
        <td class="cart__final-price text-right small--hide" data-cart-item-line-price=""><div data-cart-item-regular-price-group="">
  ...
  function Product(container) {
    ...
    this.$itemTemplate = $('[data-cart-item]')
      .first()
      .clone();
    this.$itemDiscountTemplate = $(
    ...
    this.$itemPriceListTemplate = $('[data-cart-item-price-list]', this.$itemTemplate).clone();
    this.$itemOptionTemplate = $(
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateCartPopupContent: function (item) {
      ...
      $.getJSON('/cart.js').then(
        function (cart) {
          ...
          sections.register('cart-popup-template', theme.Cart);
          this._setCartQuantity(cart.item_count);

          this._upgradeCartContent(cart);

          this._setCartCountBubble(cart.item_count);
          // $('[data-cart-popup-cart-quantity]').map((item) => item.text(cart.item_count));
          this._showCartPopup();

          if (cart.item_count > 0) {
            $('[data-cart-wrapper]').removeClass(this.classes.hide);
            $('[data-empty-page-content]').addClass(this.classes.hide);
          }
          else {
            $('[data-empty-page-content]').addClass(this.classes.hide);
            $('[data-cart-wrapper]').removeClass(this.classes.hide);
    ...
    _upgradeCartContent: function (cart, item) {
      // var key = $itemElement.attr(attributes.cartItemKey);
      // var index = $itemElement.attr(attributes.cartItemIndex);
      // var value = item.quantity;
      // remove exist cart lists
      // var emptyContent = $('[data-empty-page-content]');
      // var cartWrapper = $('[data-cart-wrapper]');
      // emptyContent.removeClass(this.classes.hide);
      // cartWrapper.addClass(this.classes.hide);

      //calculate discount
      var cartDiscountList = this._createCartDiscountList(cart)

      // create new cart lists
      $('[data-cart-line-items]').html(
        this._createLineItemList(cart)
      );

      this.setQuantityFormControllers();

      $('[data-cart-notes]', $('.cart')).val(cart.note);

      if (cartDiscountList.length === 0) {
        $('[data-cart-discount-wrapper]', $('.cart'))
          .html('')
      ...
      // } else {
      //   var $lineItem = $('[data-cart-item-key="' + key + '"]');
      //   var item = this.getItem(key, cart);

      //   $('[data-quantity-input]', $lineItem).focus();
      //   this._updateLiveRegion(item);
      // }
    ...
    _createCartDiscountList: function (cart) {
      return $.map(
        ...
        function (discount) {
          var $discount = this.$cartDiscountTemplate.clone();
          $discount.find('[data-cart-discount-title]').text(discount.title);
          $discount
            .find('[data-cart-discount-amount]')
            .html(
              theme.Currency.formatMoney(
                discount.total_allocated_amount,
                theme.moneyFormat
    ...
    _createLineItemList: function (state) {
      return $.map(
        ...
        function (item, index) {
          ...
          var $itemPriceList = $('[data-cart-item-price-list]', this.$itemTemplate).clone();
          this._setLineItemAttributes($item, item, index);
          this._setLineItemImage($item, item.featured_image);

          $('[data-cart-item-title]', $item)
            .text(item.product_title)
            .attr('href', item.url);
          ...
          $itemPriceList.html(
            this._createItemPrice(
              item.original_price,
              item.final_price,
              $('[data-cart-item-price-list]', this.$itemTemplate).clone()
            )
          );
    ...
    _setItemDiscountList: function ($item, discountList) {
      var $itemDiscountList = $('[data-cart-item-discount-list]', $item);

      if (discountList.length === 0) {
        $itemDiscountList.html('').addClass(this.classes.hide);
      } else {
        $itemDiscountList.html(discountList).removeClass(this.classes.hide);
      }
    },

    _setQuantityInputs: function ($item, item, index) {
      $('[data-quantity-input-mobile]', $item)
        .attr('id', 'updates_' + item.key)
        .attr(this.attributes.quantityItem, index + 1)
        .val(item.quantity);

      $('[data-quantity-input-desktop]', $item)
        .attr('id', 'updates_large_' + item.key)
        .attr(this.attributes.quantityItem, index + 1)
        .val(item.quantity);

      $('[data-quantity-label-mobile]', $item).attr(
        'for',
        'updates_' + item.key
      );

      $('[data-quantity-input-desktop]', $item).attr(
        'for',
        'updates_large_' + item.key
      );
    },
    ...
    _createItemDiscountList: function (item) {
      return $.map(
        ...
        function (discount) {
          ...
          $discount
            .find('[data-cart-item-discount-title]')
            .text(discount.discount_application.title);
          $discount
            .find('[data-cart-item-discount-amount]')
            .html(
              theme.Currency.formatMoney(discount.amount, theme.moneyFormat)
            );
          return $discount[0];
    ...
    _setItemPrice: function ($item, price) {
      $('[data-cart-item-price]', $item).html(price);
    },

    _setLineItemAttributes: function ($item, item, index) {
      $item
        .attr(this.attributes.cartItemKey, item.key)
        .attr(this.attributes.cartItemUrl, item.url)
        .attr(this.attributes.cartItemTitle, item.title)
        .attr(this.attributes.cartItemIndex, index + 1)
        .attr(this.attributes.cartItemQuantity, item.quantity);
    },

    _setLineItemImage: function ($item, featuredImage) {
      var $image = $('[data-cart-item-image]', $item);
      var sizedImageUrl =
        featuredImage.url !== null
          ? theme.Images.getSizedImageUrl(featuredImage.url, 'x190')
          : null;
    ...
    _getPropertyList: function (properties) {
      ...
      return $.map(
        ...
        function (property) {
          ...
          $propertyElement
            .find('[data-cart-item-property-name]')
            .text(property[0]);

          if (property[0].indexOf('/uploads/') === -1) {
            $propertyElement
              .find('[data-cart-item-property-value]')
              .text(': ' + property[1]);
          } else {
            $propertyElement
              .find('[data-cart-item-property-value]')
          ...
          $propertyElement.removeClass(this.classes.hide);
    ...
    _setProductDetailsList: function ($item, productDetailsList) {
      var $itemDetails = $('[data-cart-item-details]', $item);

      if (productDetailsList.length === 0) {
        $itemDetails.addClass(this.classes.hide).text('');
      } else {
        $itemDetails.removeClass(this.classes.hide).html(productDetailsList);
      }
    },

    _setItemRemove: function ($item, title) {
      $('[data-cart-remove]', $item).attr(
        'aria-label',
        theme.strings.removeLabel.replace('[product]', title)
      );
    },
    ...
    _createItemPrice: function (original_price, final_price, $priceTemplate) {
      if (original_price !== final_price) {
        ...
        $('[data-cart-item-original-price]', $discountedPrice).html(
          theme.Currency.formatMoney(original_price, theme.moneyFormat)
        );
        $('[data-cart-item-final-price]', $discountedPrice).html(
          theme.Currency.formatMoney(final_price, theme.moneyFormat)
        );
        $discountedPrice.removeClass(this.classes.hide);
        ...
        $('[data-cart-item-regular-price]', $regularPrice).html(
          theme.Currency.formatMoney(original_price, theme.moneyFormat)
        );

        $regularPrice.removeClass(this.classes.hide);
    ...
    _setItemLinePrice: function ($item, price) {
      $('[data-cart-item-line-price]', $item).html(price);
    },
    ...
    _updateLiveRegion: function (item) {
      var $liveRegion = $('[data-cart-status]');
      $liveRegion.html(this._liveRegionText(item)).attr('aria-hidden', false);
    ...
    _getRemoveMessage: function (lineItem) {
      ...
      var $tableCell = $('[data-cart-table-cell]', lineItem).clone();
      $tableCell
        .removeClass()
        .addClass(this.classes.cartRemovedProduct)
        .attr('colspan', '4')
        .html(formattedMessage);
    ...
    _formatRemoveMessage: function (lineItem) {
      var quantity = lineItem.data('cart-item-quantity');
      var url = lineItem.attr(this.attributes.cartItemUrl);
      var title = lineItem.attr(this.attributes.cartItemTitle);
    ...
    setQuantityFormControllers: function () {
      if (this.mql.matches) {
        $('[data-quantity-input-desktop]').attr('name', 'updates[]');
        $('[data-quantity-label-mobile]').removeAttr('name');
      } else {
        $('[data-quantity-label-mobile]').attr('name', 'updates[]');
        $('[data-quantity-input-desktop]').removeAttr('name');
      }
    },
    ...
    _setCartCountBubble: function (quantity) {
      ...
      if (quantity == 0) {
        this.$cartCountBubble.addClass(this.classes.hide);
      }
      else {
...
function switchMobile() {
  if (window.mobileCheck()) {
    $('.left.svg__item').css('display', 'none').css('align-items', 'center');
    $('.right.svg__item').css('display', 'flex').css('align-items', 'center');
    // Show gradient and left/right arrows only if scrollable
    $(".scroll-area").each((i, el) => {
      $(el).parent().find(".right.svg__item")[
    ...
    $('.left.svg__item').css('display', 'none').css('align-items', 'center');
    $('.right.svg__item').css('display', 'none').css('align-items', 'center');
  }
}
switchMobile()
...
$('.scroll-area').on('touchmove', function (e) {
  const fullWidth = $($(this)[0])[0].scrollWidth - $($(this)[0])[0].offsetWidth - 1;
  const left = $($(this)[0])[0].scrollLeft
  $(this).parent().find(".left, .left-arrow")[
    left > 36
      ? "show"
      : "hide"
  ]();
  $(this).parent().find(".right, .right-arrow")[
    left < fullWidth - 36
      ? "show"
      : "hide"
  ]();
...
$(".menu-arrow-right").on('touchstart', (e) => {
  const scroll = $(".scroll-area");
  const fullWidth = $(scroll)[0].scrollWidth - $(scroll)[0].offsetWidth - 1;
  const left = $(scroll)[0].scrollLeft;
  scroll.scrollLeft(
    left + 30
  );
  $(scroll).parent().find(".right, .right-arrow")[
    left < fullWidth
      ? "show"
      : "hide"
  ]();
  $(scroll).parent().find(".left, .left-arrow")[
    left > 0
      ? "show"
      : "hide"
  ]();
})
$(".menu-arrow-left").on('touchstart', (e) => {
  const scroll = $(".scroll-area");
  const left = $(scroll)[0].scrollLeft;
  const fullWidth = $(scroll)[0].scrollWidth - $(scroll)[0].offsetWidth - 1;
  scroll.scrollLeft(
